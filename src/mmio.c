...
static int _mmio_error(mmio_t *mmio, int code, int c_errno, const char *fmt, ...) {                    
    ...
    if (c_errno) {                                                                                     
        ...
        snprintf(mmio->error.errmsg+strlen(mmio->error.errmsg), sizeof(mmio->error.errmsg)-strlen(mmio;
    }                                                                                                  
...
int mmio_open_advanced(mmio_t *mmio, uintptr_t base, size_t size, const char *path) {                  
    ...
    /* Map memory */                                                                                   
    if ((mmio->ptr = mmap(0, mmio->aligned_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, mmio->aligned{
        int errsv = errno;                                                                             
        close(fd);                                                                                     
